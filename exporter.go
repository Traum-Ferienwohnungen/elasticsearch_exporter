...
var (
	...
	counterMetrics = map[string]string{
		...
		"indices_indexing_throttle_time_ms_total": "Throttle time for indexing in milliseconds",
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, stats := range allStats.Nodes {
		...
		e.counters["indices_indexing_throttle_time_ms_total"].WithLabelValues(allStats.ClusterName, stats.Host).Set(float64(stats.Indices.Store.ThrottleTime))
